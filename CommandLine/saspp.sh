...
Main ()
{
	...
	while [ $# -gt 0 ] ; do
		...
		-xml)	# Convert binary plotting output to XML
			GOXML="YES"
			;;
...
Usage ()
{
	...
	Where <input> is the name of the SAS4A/SASSYS-1 input deck to run.
	By default, the input file name (excluding an optional ".inp" file
	extension) will be used to construct the names of the output files
	and directory.
	    ...
	    OPTIONS:
	        ...
	        -xml        Convert binary plotting output files to XML for post-processing.

	        -bold       The output directory is "Results" plus the input file name (minus ".inp").
	                    No date or pid will be added.
	                    If that directory, already exits, it will be tar-balled or deleted.
...
Init ()
{
	...
	# SAS and the binary convesion utilities include the architecture  and OS-type in the file name
	# This convention is consisting with executables created by GNUmakefile, and also
	# the description printed for the +x option.
	# No idea if this works with mini-sas.
	NATIVE=`arch`     # e.g. "x86_64"
	OSTYPE=`uname -s` # e.g. "Linux"
	EXT="x"
	...
	SAS="sas-${OSTYPE}-${NATIVE}-release.$EXT"
	MIN="mini-${OSTYPE}-${NATIVE}.$EXT" # Maybe?
...
Setup ()
{
	...
	if [ -n "${BOLD}" ]; then
		JOB_ID="$JOB_NAM"
		JOB_DIR="$INP_DIR/Results_${JOB_ID}"			# Output directory

		# Handle case where archive directory already exists
		# If there is an output file, then create a tarball with its date
		# as part of the filename. Otherwise, just delete it.
		if [ -d "$JOB_DIR" ] ; then
			if [ -r "${JOB_DIR}/${JOB_OUT}" ]; then
				ARCHIVE_DATE=`date +%Y%m%d_%H%M%S -r "${JOB_DIR}/${JOB_OUT}"`
				ARCHIVE_NAME="Archive_${JOB_ID}_${ARCHIVE_DATE}"
				if [ -d "${ARCHIVE_NAME}" ]; then
					rm -rf "${ARCHIVE_NAME}"
				fi
				mv "$JOB_DIR" "${ARCHIVE_NAME}"
				tar -czf "${ARCHIVE_NAME}".tar.gz "${ARCHIVE_NAME}"
				rm -rf "${ARCHIVE_NAME}"
			else
				echo "Removing $JOB_DIR"
				rm -rf "$JOB_DIR"
			fi
		fi
...
GoXML ()
{
	# Convert PRIMAR-4 output to XML
	EXE_PMR4=$(which "PRIMAR4toXML-${OSTYPE}-${NATIVE}.x") || EXE_PMR4=$(dirname "${0}")/PRIMAR4toXML-Linux-x86_64.x
	if [ -x "$EXE_PMR4" ] && [ -r PRIMAR4.dat ]; then
		$EXE_PMR4 < PRIMAR4.dat > PRIMAR4.xml
	fi

	# Convert CHANNEL output to XML
	EXE_CHAN=$(which "CHANNELtoXML-${OSTYPE}-${NATIVE}.x") || EXE_CHAN=$(dirname "${0}")/CHANNELtoXML-Linux-x86_64.x
	if [ -x "$EXE_CHAN" ] && [ -r CHANNEL.dat ]; then
		$EXE_CHAN < CHANNEL.dat > CHANNEL.xml
	fi
